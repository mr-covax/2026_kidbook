...
Для каждого markdown-файла в WEB/3.1_healthy lifestyle/vrednye_privychki/articles/:
...
PAGES_DIR = os.path.join(
    REPO_ROOT, "WEB", "3.1_healthy lifestyle",
    "vrednye_privychki", "articles"
)

morph = pymorphy3.MorphAnalyzer()

# Слишком общие леммы — дают ложные срабатывания.
# Ключ = concept name, значение = set лемм, которые блокируем.
BLOCKED_SINGLE_LEMMAS_BY_CONCEPT = {
    "Недосыпание": {"сон"},  # "сон" слишком общее
    "Передозировка": {"доза"},  # "доза" слишком общее
    "Фастфуд и пищевой мусор": {"мусор"},  # может быть в других контекстах
}
...
def build_form_index(
    ...
    for pattern, concept_id, filename, is_single_form, normalized_form in raw_index:
        ...
        # сразу у нескольких понятий (например, «зависимость»).
    ...
    # Длинные паттерны первыми — чтобы «компьютерные игры» матчилась раньше «игры»
...
def build_link_pattern(form_pattern: str) -> re.Pattern:
    ...
    try:
        ...
        print(f"REGEX ERROR: pattern={form_pattern!r}")
        print(f"  full={full!r}")
        print(f"  error={e}")
        raise
...
def add_crosslinks(
    ...
    for line_idx, line in enumerate(lines):
        ...
        if line.strip().startswith("!["):
            continue
        # Пропускаем строки таблиц-заголовков
...
def main():
    ...
    if not os.path.exists(PAGES_DIR):
        ...
        print(f"Ожидаемый путь: {os.path.abspath(PAGES_DIR)}")
        sys.exit(1)
    ...
    print(f"Директория статей: {os.path.abspath(PAGES_DIR)}")
    if dry_run:
